...
**Visual builder (GP-131..135, GP-247..249, flagged).** `BUILDER_ENABLED=true` adds
...
rather than inventing one. `mode: "variable"` (GP-249) is the third block: no
provider type, no schema to fetch, described by a built-in definition derived
from the variable itself (`variableDef` — the control for its `default` follows
its declared `type`), generated into `variables.tf` by its own renderer because
`type = string` is a type expression and not a quoted string. **Pointing at one
is a reference**: `references` already means "this argument is not a literal, it
is that thing over there", so a reference to a variable renders `var.<name>`
with no attribute suffix and may fill a typed slot *or* a plain argument — which
is why `canAttach` is the one rule the form and the canvas both ask, and why a
bound argument grows a row on the card. A composition saves into a playground draft beside the
...
GP-249 variables in the Build Editor.
