 ...
 * graph traversal, no AI. Runs at the end of plan snapshot production and at
 * the end of the static diff (GP-153) — moved here from the backend so both
 * producers use the identical function.
 *
 * v1 scope (documented): only *dependents of changed nodes* are impacted. A
 * delete does not additionally propagate to its dependencies' dependents;
 * richer semantic impact is a later epic.
 */
