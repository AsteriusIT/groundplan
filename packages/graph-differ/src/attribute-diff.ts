 ...
 * Moved here from the backend (GP-153): the static diff feeds it canonical
 * attribute bags the same way GP-103's manifest comparison does, so plan-,
 * manifest- and HCL-derived rows all share one masking/truncation policy.
 *
 ...
 */
import type { AttributeDiffRow, ChangeKind } from "@groundplan/graph-parser";
