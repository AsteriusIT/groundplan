...
it("reports selection through onNodeSelect: the node on click, null on pane clear (GP-146)", async () => {
  const onNodeSelect = vi.fn();
  render(<GraphCanvas graph={graph} variant="docs" onNodeSelect={onNodeSelect} />);
  await screen.findByText("node:main");
...
it("selectedAddress drives selection from outside and reveals the node (GP-146/GP-149)", async () => {
  const { rerender } = render(
    <GraphCanvas graph={graph} variant="docs" selectedAddress={null} />,
  );
  await screen.findByText("node:main");

  rerender(
    <GraphCanvas graph={graph} variant="docs" selectedAddress="aws_s3_bucket.data" />,
  );
  // The details panel opens for the externally selected node…
  expect(await screen.findByText("Terraform address")).toBeInTheDocument();
  expect(screen.getByText("aws_s3_bucket.data")).toBeInTheDocument();
  // …and the camera frames exactly that node, not the whole graph.
  await waitFor(() =>
    expect(rfInstance.fitView).toHaveBeenCalledWith(
      expect.objectContaining({ nodes: [{ id: "aws_s3_bucket.data" }] }),
    ),
  );

  // Clearing the controlled selection closes the panel.
  rerender(<GraphCanvas graph={graph} variant="docs" selectedAddress={null} />);
  expect(screen.queryByText("Terraform address")).not.toBeInTheDocument();
});

it("an externally driven selection does not echo through onNodeSelect (GP-149)", async () => {
  const onNodeSelect = vi.fn();
  const { rerender } = render(
    <GraphCanvas graph={graph} variant="docs" onNodeSelect={onNodeSelect} selectedAddress={null} />,
  );
  await screen.findByText("node:main");
  rerender(
    <GraphCanvas graph={graph} variant="docs" onNodeSelect={onNodeSelect} selectedAddress="aws_s3_bucket.data" />,
  );
  await screen.findByText("Terraform address");
  expect(onNodeSelect).not.toHaveBeenCalled();

  // A user clearing the pane afterwards still reports null.
  fireEvent.click(screen.getByTestId("pane"));
  expect(onNodeSelect).toHaveBeenLastCalledWith(null);
});
