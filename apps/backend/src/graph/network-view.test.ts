...
// A realistic slice: vnet ⊃ subnets ⊃ vm, an NSG associated to a subnet, an
// association join, a module container, and an app resource outside the network.
const GRAPH: Graph = {
  version: 4,
  nodes: [
...
test("keeps network structure and drops modules, plumbing and non-network resources", () => {
  ...
  assert.ok(ids.has("azurerm_network_security_group.web")); // associated with a kept subnet
...
test("parent_id containment becomes contains edges for vnet/subnet containers", () => {
  ...
  // depends_on edges survive only between kept nodes.
  const deps = projected.edges.filter((e) => e.kind === "depends_on");
  assert.ok(!deps.some((e) => e.from === "azurerm_key_vault.kv"));
});

test("internet exposure propagates from the NSG to its associated nodes", () => {
  const projected = networkViewGraph(GRAPH);
  const subnet = projected.nodes.find((n) => n.id === "azurerm_subnet.web")!;
  assert.equal(subnet.internet_exposed, true);
});
...
test("the projection renders to draw.io with real network containers", async () => {
  ...
  // The exposed subnet carries the exposed stroke.
  const subnet = xml.slice(xml.indexOf('id="azurerm_subnet.web"'));
  assert.ok(subnet.slice(0, subnet.indexOf("</object>")).includes("strokeColor=#d4531e;"));
});
