...
test("a push to main auto-generates a docs snapshot for the pushed sha", async () => {
  ...
  try {
    const { projectId, repoId, token } = await createRepo(app);
...
test("duplicate push events for the same sha produce a single snapshot", async () => {
  ...
  try {
    ...
    assert.equal((await hclSnapshots(app, repoId)).length, 1);
    await app.inject({ method: "DELETE", url: `/api/v1/projects/${projectId}` });
  } finally {
    await app.close();
  }
...
test("a push to a non-default branch does nothing", async () => {
  ...
  try {
    ...
    assert.equal((await hclSnapshots(app, repoId)).length, 0);
    await app.inject({ method: "DELETE", url: `/api/v1/projects/${projectId}` });
  } finally {
    await app.close();
  }
