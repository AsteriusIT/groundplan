...
let counter = 0;
async function seedSnapshot(app: Awaited<ReturnType<typeof buildApp>>) {
  counter += 1;
...
test("export.svg renders an SVG, then serves the second request from cache", async () => {
  ...
  try {
    const { projectId, snapshotId } = await seedSnapshot(app);

    const first = await app.inject({ method: "GET", url: `/api/v1/snapshots/${snapshotId}/export.svg` });
    ...
    const second = await app.inject({ method: "GET", url: `/api/v1/snapshots/${snapshotId}/export.svg` });
...
test("export.png returns a real PNG", async () => {
  ...
  try {
    const { projectId, snapshotId } = await seedSnapshot(app);
    const res = await app.inject({ method: "GET", url: `/api/v1/snapshots/${snapshotId}/export.png` });
...
test("?scope=changes renders a smaller image than the full graph", async () => {
  ...
  try {
    const { projectId, snapshotId } = await seedSnapshot(app);
    const full = await app.inject({ method: "GET", url: `/api/v1/snapshots/${snapshotId}/export.svg?scope=full` });
    const changes = await app.inject({ method: "GET", url: `/api/v1/snapshots/${snapshotId}/export.svg?scope=changes` });
...
test("export endpoints 404 for an unknown snapshot", async () => {
  ...
  try {
    ...
    const res = await app.inject({ method: "GET", url: `/api/v1/snapshots/${missing}/export.svg` });
