...
/**
 * The building blocks of the two settings surfaces (GP-185): the personal page
 * at `/settings` (Account, Appearance — GP-187) and the organization page at
 * `/orgs/:orgId/settings` (Members, Integrations, Invitations, CI token, AI,
 * Danger zone — GP-188). Both compose these same cards and the same section
 * rail, so a card can never mean two different things on the two pages.
 */
...
export type SectionEntry = { id: string; label: string; element: ReactNode };
export type SectionGroup = { label: string | null; sections: SectionEntry[] };
 ...
 */
export function SettingsRail({ groups }: Readonly<{ groups: SectionGroup[] }>) {
...
/**
 * Stacks a page's section groups into cards. Group labels live in the rail
 * alone; here the grouping is spacing — tight within a group, wide between.
 */
  ...
  return (
    ...
    >
      ...
      {/* GP-79. Not an AI surface — it is a display preference, and it belongs
          beside the theme for the same reason the theme does: this is where
 ...
 * project page; this is the shared alternative.
 */
